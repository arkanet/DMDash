...
The same shared web app is also packaged as the DarkMesh app for iOS from the repository root. Use `pnpm mobile:sync` to copy this package into the Capacitor shell, `pnpm mobile:build:ios` to compile and package an IPA, or `pnpm mobile:ios` to open `packages/mobile/ios/App/App.xcodeproj` in Xcode. The IPA generated by the root script is copied to `packages/web/public/downloads/darkmesh.ipa`, and the AltStore source is regenerated at `packages/web/public/altstore/source.json`.
